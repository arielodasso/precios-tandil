...
export async function GET(_request: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    if (!product) return errorResponse('not_found', `Producto no encontrado: ${slug}`, 404);
    ...
    const offers = offersResult.rows.map((row) => {
      const hasData = row.captured_at != null;
      const capturedMs = hasData ? new Date(row.captured_at).getTime() : null;
      const ageHours =
      ...
      return {
        ...
        captured_at: hasData ? new Date(row.captured_at).toISOString() : null,
    ...
    if (freshOffers.length === 0) {
      ...
      const prices = freshOffers.map((o) => o.price as number);
    ...
    const eanNumber =
      product.ean !== null && /^\d+$/.test(product.ean) ? Number(product.ean) : null;
